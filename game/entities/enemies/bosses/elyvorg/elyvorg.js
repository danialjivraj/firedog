...
export * from "./elyvorgAbilities.js";
import {
    ElectricWheel, PurpleBarrier, PurpleLaserBeam, MeteorAttack, PoisonDrop,
    GhostElyvorg, GravitationalAura, InkBomb, PurpleFireball,
    BlueArrow, YellowArrow, GreenArrow, CyanArrow,
    ChargeIndicatorBalls, PurpleSlash, PurpleThunder
} from "./elyvorgAbilities.js";
...
export class Elyvorg extends EnemyBoss {
    ...
    meteorLogic() {
        ...
        if (this.meteorAnimation.frameX === 23) {
            if (this.canMeteorAttack) {
            ...
            if (this.meteorThrowCount >= this.meteorThrowTarget) {
    ...
    barrierLogic(deltaTime) {
        ...
        const barrierOffset = this.runningDirection > 0 ? 10 : -10;
        this.barrier.x = this.state === 'run'
            ? this.runAnimation.x + (this.runAnimation.width / 2) - (this.barrier.width / 2) + barrierOffset
            : this.x + (this.width / 2) - (this.barrier.width / 2);
    ...
    electricWheelLogic(deltaTime) {
        ...
        if (!this.electricWheelThrown && this.isElectricWheelActive) {
            ...
            const wheelOffset = this.shouldInvert ? 10 : -10;
            this.electricWheel.x = this.state === 'run'
                ? this.runAnimation.x + (this.runAnimation.width / 2) - (this.electricWheel.width / 2) + wheelOffset
                : this.x + (this.width / 2) - (this.electricWheel.width / 2);
