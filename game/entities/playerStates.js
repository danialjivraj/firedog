...
class State {
    ...
    gameOver() {
        ...
        if (this.game.gameOver) {
            ...
            player.setState(PlayerState.DYING, 1);
        }
    }
}
        ...
        if (anyLR(this.game, input)) {
            ...
            if (this.game.isBossVisible) {
                if (!player.isEnergyExhausted) {
                    player.setState(PlayerState.ROLLING, 0);
                }
            } else {
                player.setState(PlayerState.ROLLING, 2);
            }
        }
    }
}
...
export class Running extends State {
    ...
    handleInput(input, deltaTime) {
        ...
        if (this.game.boss.talkToBoss) {
            ...
            player.setState(PlayerState.SITTING, 0);
        }
    }
}
...
export class Jumping extends State {
    ...
    handleInput(input, deltaTime) {
        ...
        if (player.onGround()) {
            ...
            player.setState(PlayerState.FALLING, 1);
        } else if (
            (dive(this.game, input) || (sit(this.game, input) && sitEqualsDive(this.game))) &&
            player.divingTimer >= player.divingCooldown
        ) {
            if (anyLR(this.game, input)) {
                player.setState(PlayerState.DIVING, 1);
            } else {
                player.setState(PlayerState.DIVING, 0);
            }
        } else if (rollRequested(this.game, input) && !this.game.cabin.isFullyVisible) {
            if (!player.isEnergyExhausted) {
                player.setState(PlayerState.ROLLING, 2);
                ...
                if (!rollRequested(this.game, input) && player.onGround()) {
                    ...
                    if (anyLR(this.game, input)) {
                        if (player.isBluePotionActive) {
                            player.setState(PlayerState.DIVING, 4);
                        } else {
                            player.setState(PlayerState.DIVING, 2);
                        }
                    } else {
                        player.setState(PlayerState.DIVING, 0);
                    }
...
export class Standing extends State {
    ...
    handleInput(input) {
        ...
        if (anyLR(this.game, input)) {
            ...
            player.setState(PlayerState.JUMPING, 0);
        }
    }
}
...
export class Dashing extends State {
    ...
    handleInput(input) {
        ...
        if (worldStopped) {
            if (anyLR(this.game, input)) player.setState(PlayerState.RUNNING, 1);
            else player.setState(PlayerState.STANDING, 0);
        } else {
            player.setState(PlayerState.RUNNING, 1);
        }
    }
